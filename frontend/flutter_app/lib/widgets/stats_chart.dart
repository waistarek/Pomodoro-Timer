        ...
        BarChartData(
          barGroups: [
            for (var i = 0; i < items.length; i++)
              BarChartGroupData(
                x: i,
                barRods: [BarChartRodData(toY: items[i].focusMinutes.toDouble(), width: 14)],
              ),
          ],
            ...
            bottomTitles: AxisTitles(
              sideTitles: SideTitles(
                ...
                getTitlesWidget: (value, meta) {
                  ...
                  if (index < 0 || index >= items.length || index % 2 != 0) return const SizedBox.shrink();
                  final label = items[index].label;
                  return Padding(
                    padding: const EdgeInsets.only(top: 8),
                    child: Text(label.length > 5 ? label.substring(label.length - 5) : label, style: const TextStyle(fontSize: 10)),
                  );
                },
              ),
            ),
